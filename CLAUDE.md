...
- **每日任务队列 `docs/TASKS.md`（开工第一眼）**：跨站当日/本周任务的单一入口，👤/🤖 分栏 + 日期节点表；用户找"今天干嘛"只看它。它是 PLAN 的视图不是源头 —— 做完勾掉，决策数字仍回写各 PLAN；Claude 每次动文档时顺手清过期项
...
**新机会量级线（2026-09-24 用户定标）**：词族/簇**合计 ≥10K US/月** 才值得动手——新词族加页、扩容、新项目立项同用此线；已在批次车上的不追溯。几百到几千的修补型小机会（变体段/小变体页）一律不做（先例：州考变体词族 780 撤销）。

### 关键词验证纪律（每批次上页前过一遍，2026-09-05 立）

- **正式批次三样齐全**：量(US/月) + KD + SERP 前 10 实勘。实勘要落到**具体竞品名和形态**，不许写"竞争弱"三个字了事
- **没拉到量的只能进「赌注批次」**：PLAN 里单独标注缺什么数据、何时补测、补测后的判定规则（先例：chartglade 字母矩阵）
- **枚举补扫防漏词**：组词池时按"实体 × 修饰词/问法"全枚举再筛（教训：tintbrew 最大场景词 brown 49.5K 是用户找到的，我漏了）
- **SERP 弱信号识别**：Reddit/Quora/Pinterest/YouTube 占 top 5 = 可进；大厂矩阵（K5/Canva 级）垄断该修饰词 = 放弃该词不硬打
- **AI 截流分级（选词维度，2026-09-06 立）**：**答案型**词（一句能答完，what colors make X 类）被 AI Overview 截流风险最高，不当主战场；**文件/打印型**（printable / chart / graph paper）与**交互工具型**（generator：输入→定制输出）AI 替代不了，优先。实锤指标：曝光涨 + 排名进 top 10 + 点击仍零 → 确认截流，词池移向工具型
- **主题聚焦护栏（站点质量纪律，2026-09-06 立）**："杂"的判据 = **搜索人群+使用场景是否同一批**，不是页面品类。三问：①搜索者是本站核心人群吗（chartglade = 老师/家长）②打印/使用动作发生在同一场景吗（课堂）③能挂进现有 hub 或自建单一 hub 收口吗——全过才进；漂出主题的好词进**第三站候选池**不硬塞（habit tracker/planner、大学生高 CPC 均入池）。孤岛页（人群不同+无内链+为量而做）绝对禁止，DR0 新站主题聚焦是排名本钱
- **证据分级（PLAN 里每个数字/判断必带等级）**：【实测】工具拉到的量/KD ｜【实勘】SERP 亲眼所见（带竞品名+日期）｜【推断】由前两者推出 ｜【假设】未验证。只有【实测】【实勘】能支撑决策；【假设】只许出现在赌注批次且挂补测时点
- **大站做法先行（2026-09-18 立，用户批评触发）**：判定关键词前先 Google 实勘**大站是怎么服务该词的**（页面形态/内容深度/图片用法/变体结构），看完再下"可以做/不做"的结论 —— 只报"SERP 弱可打"而没拆在位者怎么做的 = 没调研完，不许拍板。新站选品固定加"**在位者 sitemap 逆向**"一步（页数/URL 分类法/图片声明数），已上线站每月复查一次（先例：Superstar 930 页挂 21,118 条图片声明 → 当日立项图片 SEO 通道）
- **分工边界（2026-09-05 验证过）**：搜索量/KD 拉数 = **用户**（浏览器 AITDK 插件，其官网无查量 API，Claude 够不到）；SERP 实勘/候选词挖掘/竞品页分析 = Claude 自动搜索。自动探针有**区域污染**：结果里出现词典/翻译站/中文界面 = 非美区 SERP，该次探针作废重勘（先例：`cursive alphabet` 探针返剑桥词典页，弃用）
