...
| 2026-09-25 | **拼豆 studio 两修上线（👤 反馈当日修，main `84712fe`）**：① **zone 取色器"自定义色点了没反应"修复** —— 根因：`.bead-picker` 弹层挂在 zone chip `<button>` 内部，点自定义 input 的 click 冒泡进 chip-toggle 分支当场拆掉弹层（连同 input），原生取色器根本没机会开。修法四件：弹层内点击吞掉不落 chip 切换 ｜ 自定义行加 **Use color 确定钮**（原 label 改 div，防 label 把按钮点击转发给 input）｜ 回显当前选中（网格 active 高亮 + input 初始值 = 当前 zone 色 + hint 显示 hex→nearest）｜ 补监听 `change`（部分浏览器只在原生对话框确认时触发）。② **Blank canvas 加坐标**（👤 "格子无法辨别位置"）：SVG 顶/左各留 6 viewBox 单位边距，每 5 格标数字（猫页 = 列 5..55 / 行 5..40，等宽字体，随图打印——纸上数位更有用）；island `cellAt` 改为从 viewBox 属性自推边距与缩放，不硬编码。**测试 142→145**（+3 zone 弹层回归：active 回显 / 点 input 不关弹层 / 自定义 hex 实时吸附 + Use 提交收口；px() 助手按新 viewBox 换算 + 坐标 label 断言），96 页 build 绿 |
| 2026-09-25 | **图纸描边完整性 + 画布坐标可读性两连修（👤 反馈"Reference 并不对，Bear 的 outline 不是一圈黑""坐标字太小看不清"→ 根因均在引擎/架构层，main `85aa129`）**：① **K 黑圈补全**：两处引擎真 bug —— rim 扫描原边转换边写，下一个格子看到的是**已转黑的邻居**（空邻居被藏起来），熊耳侧边描边漏一圈成 scan-order 破洞 → 改"先按 pre-rim 网格定标记、扫完统一落子"；dropSingles 原 4 邻域，描边楼梯是斜连接 → 删掉每个对角拐角珠，改 8 邻域。另 18 处 rim 区声明补丁（animals/food/christmas）+ 4 处几何修：熊阴影月牙外弧坐在剪影上（右半圈描边永远不黑）→ inset+clip 进头内、圣诞帽雪花移位、铃铛高光出圈、chick 'Y'/'y' 大小写漏阴影。**46 图全审计**：37 张 K 圈完整过，9 张记录为设计留白（幽灵下摆、月牙 C 口、斜比萨等）。unicorn 字节守卫过（引擎改动未碰试点）。② **坐标可读性**（根因是一摞布局 bug，非字号）：Astro scoped 样式**不跨组件边界** —— 画布卡借了页面模板的 .fig-card/.bead-fig/.bead-list 样式从未生效（卡片上线起就没边框没内边距、svg 退到固有 ~416px = 0.72 缩放，11px 字号实际渲染 8px；列表渲染成默认圆点）；画布卡自身 paint-body 的 320px 侧栏又把图压在 416px。修法：BeadCanvas 自给自足（scoped fig-card/bead-fig + 全局 cv-list 药丸样式）+ 画布全宽、工具两列排下方 + gutter G 14→16、字号 12px 粗体 0.8 墨色 → 实测标签 **15.1px**（原 8px）。**③ 光标坐标徽章**（👤 "brush 时光标上显示坐标"）：.cv-cursor 悬浮 readout 跟指针显示 1 基格坐标，出网格即隐。**验证方法论两课**：视觉模型两次说"字太小"但 CSS 真在生效 → headless 量 computed 链（main 1152→canvas 416 找到真凶），**量不猜**；scrollIntoView(block:start) 会把目标顶进 sticky header 底下，截图前必 block:center。测试 146 全绿（tmp 审计测试移除），.gitignore 加 `**/tmp/`。上线后 curl 实勘：线上 bear HTML 与本地 dist **md5 一致**、cv-label 12px/700/#000c、viewBox gutter 16 ✓；IndexNow 补推 51 perler URL 返 200【实测】 |
| 2026-09-25 | **模板卡按视口高度自适应 + 浮层外部点击关闭修复（👤 三次反馈 bear 页"图案+横排颜色还是放不下"—— 408px 卡 + 291px 浮层 > 短视口，main `3868cc7`）**：固定浮层后余量数学 = 卡高 + 浮层 ≤ 视口，卡按卡片宽度生长（~408px）在 707 视口正好差一点点、600 视口必超。修法：studio 两卡的 svg 加 `max-height:max(216px, 100dvh - 385px)` + `width/height:auto`（保比例居中 letterbox，预留 = 浮层 291 + 底距 16 + 卡 chrome 64 + 余隙）——**大屏宽度约束不变（1000 视口 svg 334 不缩），短屏自动缩到整卡+浮层必然同屏**；浮层自身再压 ~20px（padding 12→10、gap 6→5）。规则放 `is:global`（svg 在 BeadFigure 内，scoped 不跨边界）。**连带修真 bug**（探针 600 视口轮浮层开不出来暴露）：监听挂 studio 根 → 点击 studio 之外（hero/正文/footer）永远关不掉浮层 → 加 document 级 click（studio 内点击让根 handler 管，closePicker 幂等）。**量化验收【实测 CDP 探针 bear 页三视口，点末位芯片+卡顶对齐】**：707 卡 4-391 vs 浮层 400（9px 余隙）/ 600 卡 4-285 vs 浮层 293（8px 余隙，svg 落 floor 216）/ 1000 卡 403 不缩（宽度约束）。视觉复核过（两卡完整并排浮层上方、字母可辨）。测试 147 绿；线上 bear+panda md5 与 dist 一致【实测】；IndexNow 51 URL 200【实测】。**全站审计【实测 CDP，👤 要求"其他图案也要检查"】**：46 页 × 707/600 双视口全流程（点末位芯片开浮层 + 卡顶对齐）**46/46×2 全 PASS 零例外** —— 707 视口 41 页落 322 帽、5 页宽度约束不缩（watermelon 2.60 最横 195 高 / dog·bat 1.75 / burger 1.81 / chick 1.6），600 视口 45 页落 216 下限全部 8px+ 余隙；极端比例两端（最竖 mini-flower 0.80 / 最横 watermelon 2.60）均无挤压无溢出 |
| 2026-09-25 | **取色弹层改固定底部浮层（👤 二次反馈 panda 页"往下拉选颜色就看不到图案了"—— 上一版根修不彻底，main `3ceffe0`）**：`29e189c` 把选色面板挪到模板卡下方只解决了"找到 zone"，但弹层仍内嵌在芯片**正下方**占文档流 —— 要点到色板就得再往下滚，一滚模板又出屏（布局自搏：模板下方的一切都在和模板抢视口）。**终修 = 弹层不占流**：`.bead-picker` 改 `position:fixed` 底部居中浮层（`width:min(440px,100vw-32px)`、z-index 60），挂到 studio 根而非 chip 内 —— 永远全屏可见、**打开零位移、滚动中常驻**，模板可随时滚到浮层上方完整观看；浮层自带 zone 名标题（如 "blush zone"，脱离芯片后必须自述）；顺带修掉 chip `<button>` 内嵌按钮的非法 HTML。点模板选区的自动滚动删除（旧滚动只为给内嵌弹层腾位，恰是反向力）。前置校验：`.studio` 祖先链无 transform/filter，fixed 不会被劫持成局部坐标系。**量化验收【实测 CDP 探针 panda 页 707px 视口】**：点最后一个芯片 blush —— scrollY 666→666 零位移、浮层 y395-691 全屏内、32 色全；点模板 cell —— scrollY 235→235 零位移。视觉复核两场景过（4.5v 远程模型）。测试 147 绿（bead.test 弹层查询从 chip 内改 studio 根）。线上 panda+bear md5 与 dist 一致【实测】；IndexNow 51 URL 200【实测】 |
| 2026-09-25 | **选色面板挪到模板卡正下方（👤 反馈"选 nose 时已经看不到图片效果"，main `29e189c`）**：根因 = zone 选色器在 320px 右栏且右栏比旁边的模板卡**高** —— 滚到列表低位 zone（熊 nose）时，被重配色的模板早已滚出屏幕顶，选色瞬间恰好看不见效果。修法：studio 改**单列**（Reference + Color it yourself 两卡并排在上，选色面板全宽紧贴其下；≤900px 两卡也叠单列）；面板同步压扁 —— Reset/Print 按钮并进标题行（.studio-head flex）、zone chips 多列网格 `minmax(240px,1fr)`、说明文字挪到 chips 下方（弹层向下开会盖住它，正好不偷模板高度）、弹层加 max-width 420px（chips 比旧 320px 栏宽，防色珠撑巨大）；点模板选 zone 的滚动由 `scrollIntoView('nearest')` 改 scrollBy 向下预留 250px 弹层位（弹层只在点外部/换 zone 时关，滚动中常驻，矮视口余量可手动补滚）。**量化验收【实测 CDP 探针】**：bear 页 707px 视口点 nose 芯片 —— nose 色区 y113-120 / chip y389-439 / 弹层 y444-623 / 模板卡可见 62%，同屏共见；子块顺序 grid→side→paint→list。视觉复核过（4.5v 远程视觉模型；本机 zai MCP 401 沿用绕行）：两卡并排、面板全宽在下、32 色珠+自定义行完整。测试 147 绿（markup 压缩后重跑）；线上 bear+skull md5 与 dist 一致【实测】；IndexNow 51 URL 200【实测】。**CDP 坑入档**：裸 ws + 单消息处理器 + readyState 轮询 = 稳；loadEventFired 事件 Promise + 第二个监听器的结构会挂死（Runtime.evaluate 永不回包，最小复现 cdp-ping 排除法定位） |
| 2026-09-25 | **画布调色板同屏 + 严格镜像对称 + Mirror 笔刷三连（👤 反馈"canvas 太大选色不同屏""对称坐标不同色"，main `af9f6a3`）**：① **同屏排版**：paint-body ≥901px 改 `minmax(0,1fr) 280px` 双栏（画布左/工具右，调色板 sticky 跟随），画布 svg 上限 1.25×viewBox 宽（大图坐标字稳 ~15px、迷你图不再撑满全宽）；paint-card 跨 studio 两栏全宽。**连带修掉一个隐藏炸弹**：Chrome 对 sticky grid item 的约束矩形是**整个 grid 容器**而非其 grid area —— zone 侧栏一路粘到 studio 底部，整块盖在画布工具栏上双影透字（视觉模型报"重叠"先被当误报，量包围盒实锤后 872,12 处 320×776 全盖 912 起的工具列）→ zone 侧栏去 sticky（画布自带调色板，zone chips 滚过模板区即失效）。② **严格镜像对称**：👤 报"对称点 11,16/46,16 不同色"——实测其坐标出自 skull 页（52 宽，11 的真镜像是 42，46 已在图形缘外；肉眼找镜像点天然会偏，坐标每 5 格才标一个数）。但全站 46 图逐格镜像对拍揪出 **781 格真不对称**：分类后 5 张是引擎伪影（超采样 50/50 平票两侧各自取整相反 + def 圆心半格偏轴：bunny 脸缘 1 格/chick 呆毛+体缘/penguin 缺左鳍 3 格/草莓叶冠 11 格/寿司海苔左 6 右 5）→ **引擎加对称化终检**（mirror:true 的 def 在 rim 前对占位做镜像 OR，格子只会获得镜像伙伴的区域字符、绝不丢格；单侧阴影等内部设计不受影响；`asymOk` 白名单留设计例外）+ strawberry/sushi 误标 mirror:false 纠正 + unicorn 鬃毛锁 3 格入白名单（字节守卫过）。其余 12 张不对称全部有设计意图在档（狗尾/比萨斜边/月牙 C/仙人掌单臂/圣诞袜斜挂/花环蝴蝶结/热可可把手/蝙蝠角月/迷你花侧叶/蘑菇斑点 + panda 瞳孔看左）。修后镜像对拍：mirror:true 全干净，视觉复核企鹅双鳍/草莓叶冠/小鸡对称呆毛全过。③ **Mirror 笔刷**（把投诉变功能）：工具行加 Mirror 开关，笔画/橡皮同步落镜像格，对称图一半笔画画完。测试 146→147 绿；线上 md5 与 dist 一致【实测】；IndexNow 51 URL 再推 200【实测】。**坑入档（CDN 截图缓存）**：Read 上传图走 CDN 且**按内容去重到同一 URL 就回旧字节**（同名文件 + 相同布局的确定性截图字节相同 → 视觉模型连续两轮看旧图报假缺陷）；解法 = 换视口尺寸/换文件名让字节必不同 + curl md5 对拍 CDN 字节后再送评审 |
