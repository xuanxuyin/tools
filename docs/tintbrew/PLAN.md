...
| 8 | **V2.2 内容扩容两族（9/24 立项，开建等 10-01 门）**：① make-X 头词 6 页（blue/red/black/maroon/peach/turquoise —— scenarios.ts 加条 + 薄 wrapper；blue/red 走"原色陷阱"文案角度：真做不出来 + shade/近似路线表）② 穿搭配色族首批 4 页（brown/green/purple/burgundy —— 新色板模板：Oklab 真渲染 swatch 网格 + hex + outfit/decor 双场景分段 + print CSS 可撕色卡） | Claude | 10 页上线 + 测试全绿 + `npm run og` 重生成（scenarios 加条后 OG 快照同步） | 10-01 门通过后启动 |
