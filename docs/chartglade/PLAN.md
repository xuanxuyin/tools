...
| 17 | **图片 SEO 通道**（9/18 MiriCanvas 拆解唯一实招，细节见 [BENCHMARKS.md](./BENCHMARKS.md)）：核心页生成真实 PNG 内容图（复用 `npm run pins` 无头 Edge 管线截 `.printable` 区域）+ 描述性文件名 + alt 从 PageDef 程序化生成（"内容+年级+用途"句式）+ 图放正文 preview 位（print 隐藏零影响）—— 打开 Google Images 流量口（现全站 HTML 表格/SVG 图片搜索收不到）；首期挑 4~6 页（cursive-alphabet / place-value / multiplication / kindergarten sight words + 2 高量字母页）；连带：10/05 合并后首页加 Halloween 季节板块 | Claude | 核心页正文含真图 `<img>` + alt/文件名规范 + build/测试绿（不加页不破止损纪律） | **10/05 批搭车**（随 v1.6 合并同批部署） |
