...
| 2026-09-18 | **#17 图片 SEO 当日开发上线（main `c91c5ac`，走 #13"只加内容不加页"先例不破批次门；待 push 部署）**：5 支柱页（multiplication / place-value / kindergarten sight words / cursive-alphabet / alphabet-chart）正文内嵌 `.printable` 真图 PNG（2016px 宽 37~148KB、lazy、描述性文件名、手写 alt"内容+形态+用途"句式、`no-print` 打印零影响）；新管线 `npm run sheet-images`（本地 http 服务 dist + 无头 Edge @2x，自托管字体真实渲染，Dancing Script 目检过）；+6 守卫测试（声明尺寸必须与磁盘 PNG 一致）。main 53 页 29 测试绿 / v1.6-predev 65 页 61 测试绿。后续：字母页/变体页分批补图；Google Images 收录效果 10 月中旬随 10/17 验收复盘 |
