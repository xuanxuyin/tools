...
| 2026-09-24 | **#18 支柱页下载按钮开发完成（predev `ece2230`，上线随 10/05 批合并）**：工具条 `<a download>` 零脚本、**由 sheetImage 数据驱动** —— 恰好 5 支柱页带钮，#17 未来补图的页自动继承；SeoHead 非 noindex 页显式 `max-image-preview:large`（404 保持 noindex,nofollow）；6 条 dist 守卫测试（5 页锚点在位 + 无图页零污染），**67 测试全绿 + 65 页构建过**。**顺带入档一个文案张力（10/05 批定夺）**：多处 meta/FAQ 写 "no download, no sign-up"（本意 = 无强制下载/无邮件墙），现在页上有下载钮字面略拧巴 —— 5 支柱页文案是否软化为 "no sign-up" 待裁，不在 #18 范围擅改 |
| 2026-09-24 | **V1.8 sign-in sheet 族开发完成（predev `76d1136`，上线随 10/05 批合并）**：hub + 5 页全量 —— plain（22 行 name/time in/out/signature）、open house（18 行 parent/student/email/phone）、PT conference（16 行 time slot 制）、field trip（26 行 dense 紧排 + emergency contact/medication 列）、volunteer（20 行 hours 累计）；每页 meta 手写线（teacher/date/room 场景化）+ 一页一纸打印排版。**新基建**：`SignInSheet.astro` 通用组件（columns/metaFields/rows/dense 全数据驱动）+ `SHEET_LAYOUTS` 配置表 + pageTypes/HUBS/Footer 接线（Footer 走 ChartGlade 列 halloween 先例，不进首页卡）。11 条族守卫测试（行列数夹在 3~5 列/14~28 行防溢纸 + SEO 文案 + related 全族内），**78 测试全绿 + 71 页构建过**。10/05 批 predev 待合并内容至此齐：#18 + V1.8 |
...
| 11 | **V1.8 sign in sheet 族**：/sign-in-sheets/ hub + /open-house-sign-in-sheet/（3.6K 头名）+ /sign-in-sheet/（template+printable 6.7K 教师角度通用页）+ PT conference / field trip 变体页；visitor/attendance 只做 FAQ 接住 | Claude | hub + 4~5 页，print 剥纸验证，测试全绿 | ✅ **9/24 开发完成 predev `76d1136`**（hub+5 页+组件+11 测试，78 绿）——原 **10/05 批（9/24 用户指令提前**，原 11 月：赶 10 月中~11 月 PT conference 季 + 收录需 2~4 周；⚠️ open house 3.6K 头名今年黄金窗 8~9 月已过，此批吃长尾+模板常青量）|
...
| 18 | **支柱页下载按钮**（9/18 Suncatcher 拆解产出，[BENCHMARKS.md](./BENCHMARKS.md) §3）：5 支柱页 `.printable` 工具条加 "Download PNG"（直接复用 #17 已上线 2016px PNG，`download` 属性零脚本）+ SeoHead 显式 `max-image-preview:large`（保险项一行）—— PNG/PDF 下载是行业标配（Suncatcher/classweekly/InkPx 全有），手机用户（Pinterest 9 成手机）无 Ctrl+P，下载再打印是真实路径；PDF 下载 = sheet-images 管线加 page.pdf() 再评估可后置 | Claude | **✅ 开发完成 2026-09-24（predev `ece2230`）**：sheetImage 数据驱动（#17 补图页自动带钮）+ max-image-preview:large + 6 条 dist 守卫（67 测试绿）；上线 = 随 10/05 批合并 | **2026-10-05 批**（开发 9 月下旬） |
