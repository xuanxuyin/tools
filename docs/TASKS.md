...
- [ ] 👤 **AITDK 拉词五件**（9/18 起，④⑤ 9/24 新增）：① fondant 族（how to make brown fondant 等，≥500/月 才进 tintbrew 烘焙簇扩展）② states require cursive（chartglade PLAN #16 词记录）③ **halloween 三词**（word search / color by number / cursive —— 万圣簇量级补测，按三档规则定加码或缩簇）④ ~~色觉敏锐度 12 词~~ **已收档（9/24 主词三件拉量 + SERP 实勘即定案）**：color perception test 1,600/KD51 唯一过线但 X-Rite+EnChroma 锁前排 → **降级观察池**（IQ test 140 / how well 20 无量；B/C/D 组不补拉，数据已足）。详见 tintbrew PLAN 9/24 两行⑤（可选）regents formula sheet / STAAR formula sheet（chartglade V1.7 州考变体加码判定，≥500/月 才谈）
