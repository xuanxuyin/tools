...
- [ ] 👤 **AITDK 拉词五件**（9/18 起，④⑤ 9/24 新增）：① fondant 族（how to make brown fondant 等，≥500/月 才进 tintbrew 烘焙簇扩展）② states require cursive（chartglade PLAN #16 词记录）③ **halloween 三词**（word search / color by number / cursive —— 万圣簇量级补测，按三档规则定加码或缩簇）④ **色觉敏锐度 12 词全枚举**（tintbrew 候选，9/24 已发用户拉量）：A 主词 = color IQ test / color perception test / how well do you see color；B 学名族 = farnsworth munsell 100 hue test / hue test / color discrimination test；C 变体 = color acuity test / color sensitivity test / color matching test；D 对照组（预期否决画边界）= color blind test / color vision test / ishihara test。判定：ABC 组 ≥500 US+SERP 无 DR60+ 进实锤，300~500 SERP 全小站可并页，D 组 SERP 医疗垄断即弃；过线词 → Claude 美区 SERP 实勘⑤（可选）regents formula sheet / STAAR formula sheet（chartglade V1.7 州考变体加码判定，≥500/月 才谈）
