...
1. GSC 左侧 → **效果（Performance）**：默认看总点击/曝光，顶部把日期范围切到 **最近 28 天**
2. 切到 **查询（Queries）** 标签 —— 这就是"哪些词开始有曝光"的答案，新冒头的词记入 PLAN §2
3. 点任一查询词 → 切回"网页"标签交叉看：这个词落在哪个页上
4. 前 1~2 个月没数据/个位数都是正常（沙盒期），记数字即可，不动作

### ③ 周五批次决策（对应各 PLAN §6 决策点）

1. 效果 → 查询 → 28 天，对照 PLAN 目标阶梯的预期区间
2. 达标 → 按 PLAN §3 推进下一批；不达标 → 执行 PLAN §6 表格里对应条件动作（不硬扛）
...
- [x] **Email Routing（2026-09-05 完成）** —— `hello@chartglade.com` 已可收信（步骤留档，第三个站照抄）：
